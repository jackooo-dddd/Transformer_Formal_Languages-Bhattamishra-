...
class TransformerModel(nn.Module):
	...
	def forward(self, src, has_mask=True, get_attns = False, get_encoder_reps = False):
		"""
		    ...
		    This method takes a batch of token indices, applies embedding and positional
		    encoding, runs it through a multi‐layer Transformer encoder, and decodes
		    the output to prediction probabilities. Optionally, it can also return the
		    per‐layer attention weights or the raw encoder representations.
		    Args:
		        src (Tensor): Input token indices of shape (seq_len, batch_size), where
		                      each value is in [0, ntoken).
		        has_mask (bool, optional): If True, applies a causal mask so that
		                      each position can only attend to current and previous
		                      positions.
		        get_attns (bool, optional): If True, also return a list of attention
		                      weight tensors from each encoder layer.
		        get_encoder_reps (bool, optional): If True, also return the raw
		                      encoder output (hidden states) before the final decoder.
		    """
		# print("MyFlag2")
		if has_mask: ## Build a causal mask to prevent tokens from attending to future positions
			device = src.device
		...
		if self.pos_encode: ## Add PE
			src = self.pos_encoder(src)
		if get_attns: ## (Optional) Extract per‐layer attention weights for visualization
			attns = []
			...
			for layer in encoder_layers:
				...
				attns.append(attn)
		## Run the standard Transformer encoder stack
		transformer_output = self.transformer_encoder(src, self.src_mask)
		## Project to the target space and apply sigmoid
		output = self.decoder(transformer_output)
		output = self.sigmoid(output)
		# print(src.shape, output.shape)
		"""
		MIGHT ADD STACK AUGMENTATION OVER HERE.
		Another place: See the utils.py in folder components.
		"""
		if get_attns:
			return output, attns
		if get_encoder_reps:
