...
class Sampler(object):
	...
	def get_batch(self, i):
		"""
		    Construct a single training batch of input token indices and next-character targets.
		    Args:
		        i (int):  Index of the first example in this batch (0-based).
		    Returns:
		        source (LongTensor): Input token indices of shape (seq_len, batch_size),
		            where seq_len is the maximum sequence length in this batch,
		            and batch_size = min(self.batch_size, len(self.data) - 1 - i).
		            Each column corresponds to one training string (excluding its final token).
		        target (FloatTensor): One-hot (or soft) next-character targets of shape
		            (seq_len, batch_size, noutputs).  At timestep t, target[t, b, :]
		            is the one-hot vector for the character that follows source[t, b].
		        word_lens (LongTensor): Tensor of length (batch_size,) containing the
		            true lengths of each unpadded string in this batch.

		    Notes:
		        - `self.data` is a list of positive-language strings.
		        - `self.targets` is the corresponding list of “next-character” strings
		    """
		...
		## source: the input sequence of character indices
		source = batch_ids[:,:-1].transpose(0,1)
		max_length = word_lens.max().item()
		"""
		targets: a one‑hot (or probabilistic) tensor of the next character at each position
		self.Lang.lineToTensorOutput(line)[:len(word)] produces a tensor of one‐hot vectors corresponding exactly to the next characters
		"""
